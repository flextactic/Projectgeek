...
route.post('/add',auth,async(req,res)=>{
  try{
    ...
    if(projectData.Author!=req.user._id) return res.send(400).send(`you don't have proper rights to add this project in requirement section`);
    ...
    user.projectInRequirement.push({id:req.body.projectId,reqDescription:req.body.description});
    user.save();
...
route.get('/me',auth, async(req,res)=>{
  try{
    const projectInRequirement= await ProjectInRequirement.find({authorID:req.user._id}).populate('projectID');
    if(!projectInRequirement)  return res.status(400).send('NO project exist for this user.');
    res.status(200).send(projectInRequirement);
  }
  catch(ex)
  {
...
route.put('/upvote', auth, async(req,res)=>{
  //TODO: Refactor this route using transactions
  try{
    const upvoted =await User.findOne({_id:req.user._id,upvoted:{id:req.body.id}});
    const downvoted= await User.findOne({_id:req.user._id,downvoted:{id:req.body.id}});
    let user, project;
    if(downvoted)
    {
       user = await User.updateOne(
        ...
        {
          $pull: {
            downvoted: { id: req.body.id, contentType: mongoose.Schema.Types.ObjectId },
          },
          $push :{
            upvoted:{id:req.body.id,contentType: mongoose.Schema.Types.ObjectId}
          },
        }
      );
